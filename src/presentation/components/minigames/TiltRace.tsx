  ...
  return (
    <div className="min-h-[100dvh] bg-sky-950 flex flex-col items-center justify-center p-6 relative overflow-hidden z-[9999] select-none">
      <div className="flex justify-between w-full max-w-sm mb-4 relative z-10">
        <div><span className="text-sky-400 text-sm font-bold">수집</span><div className="text-white text-2xl font-black">⭐ ×{collected}</div></div>
        <div className="text-right"><span className="text-sky-400 text-sm font-bold">남은 시간</span><div className={`text-2xl font-black ${timeLeft <= 5 ? 'text-red-500' : 'text-white'}`}>{timeLeft}초</div></div>
      </div>
      <h1 className="text-2xl font-black text-white mb-2 text-center relative z-10">📱 기울기 레이스</h1>
      <p className="text-sky-300 font-bold mb-4 text-center text-xs relative z-10">폰을 좌우로 기울여 공을 별에 맞추세요!</p>
      ...
      {finished && <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center"><div className="text-6xl font-black text-sky-400 mb-4">⭐ {collected}개!</div><p className="text-xl text-white font-bold">+{collected * 50}점</p></div>}
    </div>
  );
};
