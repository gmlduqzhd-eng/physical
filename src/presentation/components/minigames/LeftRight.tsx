  ...
  return (
    <div className={`min-h-[100dvh] bg-indigo-950 flex flex-col items-center justify-center p-6 relative overflow-hidden z-[9999] select-none transition-colors duration-75 ${flash === 'correct' ? '!bg-emerald-950' : flash === 'wrong' ? '!bg-red-950' : ''}`}>
      <div className="flex justify-between w-full max-w-sm mb-6 relative z-10">
        <div><span className="text-indigo-400 text-sm font-bold">점수</span><div className="text-white text-3xl font-black">{score}</div></div>
        <div className="text-right"><span className="text-indigo-400 text-sm font-bold">남은 시간</span><div className={`text-3xl font-black ${timeLeft <= 5 ? 'text-red-500' : 'text-white'}`}>{timeLeft}초</div></div>
      </div>

      <h1 className="text-3xl font-black text-white mb-2 text-center relative z-10">좌우 반사신경</h1>
      <p className="text-indigo-300 font-bold mb-8 text-center relative z-10 text-sm">공이 나타난 쪽 버튼을 터치하세요!</p>
      ...
      {finished && (
        <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center backdrop-blur-sm">
          <div className="text-6xl font-black text-indigo-400 mb-4">{score}번 성공!</div>
