  ...
  return (
    ...
    >
      <div className="flex justify-between w-full max-w-sm mb-6 relative z-10">
        <div><span className="text-cyan-300 text-sm font-bold">걸음수</span><div className="text-white text-3xl font-black">{steps}</div></div>
        <div className="text-right"><span className="text-cyan-300 text-sm font-bold">남은 시간</span><div className={`text-3xl font-black ${timeLeft <= 5 ? 'text-red-500' : 'text-white'}`}>{timeLeft}초</div></div>
      </div>
      ...
      <p className="text-cyan-200 font-bold mb-4 text-center text-sm">폰을 손에 들고 제자리에서 뛰거나 화면을 연타하세요!</p>
      ...
      <p className="text-cyan-300 text-xs mt-2 font-bold">목표: 50걸음! (화면 어디든 터치 가능)</p>
      {finished && <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center"><div className="text-6xl font-black text-cyan-300 mb-4">{steps}걸음!</div><p className="text-xl text-white font-bold">+{steps * 10}점</p></div>}
    </div>
  );
};
