  ...
  return (
    <div className="min-h-[100dvh] bg-pink-950 flex flex-col items-center justify-center p-6 relative overflow-hidden z-[9999] select-none">
      <div className="flex justify-between w-full max-w-sm mb-6 relative z-10">
        <div><span className="text-pink-400 text-sm font-bold">흔들기</span><div className="text-white text-3xl font-black">{waves}</div></div>
        <div className="text-right"><span className="text-pink-400 text-sm font-bold">남은 시간</span><div className={`text-3xl font-black ${timeLeft <= 5 ? 'text-red-500' : 'text-white'}`}>{timeLeft}초</div></div>
      </div>
      ...
      <p className="text-pink-300 font-bold text-center text-sm">폰을 좌우로 크게 흔드세요!</p>
      {finished && <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center"><div className="text-6xl font-black text-pink-400 mb-4">{waves}회!</div><p className="text-xl text-white font-bold">+{waves * 15}점</p></div>}
    </div>);
};
