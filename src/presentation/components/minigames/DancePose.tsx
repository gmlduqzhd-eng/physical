  ...
  return (
    <div className="min-h-[100dvh] bg-fuchsia-950 flex flex-col items-center justify-center p-6 relative overflow-hidden z-[9999] select-none">
      <div className="flex justify-between w-full max-w-sm mb-4 relative z-10">
        <div><span className="text-fuchsia-400 text-sm font-bold">라운드</span><div className="text-white text-2xl font-black">{Math.min(round + 1, TOTAL_ROUNDS)}/{TOTAL_ROUNDS}</div></div>
        <div className="text-right"><span className="text-fuchsia-400 text-sm font-bold">점수</span><div className="text-white text-2xl font-black">{score}</div></div>
      </div>
      ...
      {!finished && (
        <>
          ...
          <div className="text-3xl font-black text-fuchsia-300 mb-2 text-center">{pose.name}</div>
          <p className="text-fuchsia-400 font-bold text-sm mb-6">{pose.desc}</p>
      ...
      {finished && <div className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center"><div className="text-5xl font-black text-fuchsia-400 mb-4">총 {score}점!</div><p className="text-xl text-white font-bold">{score >= 300 ? '완벽한 댄서!' : '좋은 움직임!'}</p></div>}
    </div>
  );
};
